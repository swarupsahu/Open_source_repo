...
public class Prey extends Animal { 
	...
	public void seePredator(Predator predator, double pct) { 
	...
	public void move () { 
		...
		if ((World.listPredators.size() == 0) & Helper.nextDouble() <= .1) {
			animalDirection = Helper.nextInt(4);
		}
		...
		/** 0 indicates north, 1 indicates east, 2 indicates south, 3 indicates west 
		If the animal reaches the edge then turn in another direction and keep going. **/		
	...
	**/
	public boolean preyDeath (Predator predator, double pct) {
		...
		if (Helper.compareColorTo(World.canvasColor, this.getColor(), pct)) {
			return(false);
		} else if (predator.getY() == yPos & Helper.abs(predator.getX() - xPos) <= 2) {
			preyCount --; 
			predator.preyEaten();
			return(true);
		} else if (predator.getX() == xPos & Helper.abs(predator.getY() - yPos) <= 2) { 
			preyCount --;
			predator.preyEaten();
			return(true); 
		} else { 
