...
public class World {
	...
	**/
	...
	protected static double predatorReproductionRate = .00015; 
	protected static double predatorDeathRate = .0003; 
	protected static int predatorMaxChildren = 3;

	protected static double preyReproductionRate = .0025;
		...
		for (int i = 0; i < listPredators.size(); i++) { 
			...
			if (currentPredator.predatorDeath()){
				...
				for (int j = 0; j < listPreys.size(); j++) { 
					...
					currentPredator.seePrey(currentPrey, 20.0);
				}
			}
		}	
		...
		**/
		...
		for (int i = 0; i < listPreys.size(); i++) { 
			...
			for (int j = 0; j < listPredators.size(); j++) { 
				...
				currentPrey.seePredator(currentPredator, 20.0);
				if (currentPrey.preyDeath(currentPredator, 20.0)) {
					listPreys.remove(i); 
				}
			}
		}
